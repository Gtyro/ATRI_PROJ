...
# 词云命令
wordcloud_cmd = register_command(
    "wordcloud",
    aliases={"词云"},
    role="superuser",
    permission=SUPERUSER,
    priority=5,
    description="查看会话热门词汇统计",
    usage="wordcloud [可选会话ID] [可选数量限制]",
    examples=["wordcloud", "wordcloud 123456 30", "词云 30"],
)
...
# 列出所有会话词云命令
wordcloud_list_cmd = register_command(
    "wordcloud_list",
    aliases={"词云列表"},
    role="superuser",
    permission=SUPERUSER,
    priority=5,
    description="列出已有词云数据的会话",
    usage="wordcloud_list",
    examples=["wordcloud_list", "词云列表"],
)
