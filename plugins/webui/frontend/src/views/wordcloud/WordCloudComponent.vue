<template>
  <div class="wordcloud-container" ref="cloudContainer">
    ...
    <div
      v-if="config.controlPanel.show"
      :class="[
        'control-panel',
        `control-panel-${config.controlPanel.position}`,
      ]"
    >
      <div
        v-if="config.controlPanel.showConversationSelector"
      ...
      <el-button
        v-if="config.controlPanel.showRefreshButton"
        @click="refreshData"
        size="small"
        type="primary"
        :loading="refreshing"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const limit = ref(
      props.initialLimit || config.value.controlPanel.defaultLimit,
    );
            ...
            textStyle: {
              ...
              color: function () {
                return config.value.cloudOptions.colors[
                  Math.floor(
                    Math.random() * config.value.cloudOptions.colors.length,
                  )
                ];
              },
            },
