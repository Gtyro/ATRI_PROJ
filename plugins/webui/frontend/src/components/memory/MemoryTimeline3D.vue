...
const getTimelineData = () => {
  // 按时间排序
  const sorted = [...props.memories].sort(
    (a, b) => a.created_at - b.created_at,
  );

  // 计算时间范围
  if (sorted.length === 0)
    return { sorted, minTime: 0, maxTime: 0, timeRange: 0 };
...
const updateMemoryObjects = () => {
  ...
  sorted.forEach((memory, index) => {
    // 计算时间轴上的位置
    const timePosition = timeRange
      ? -40 + ((memory.created_at - minTime) / timeRange) * 80
      : 0;
...
const onMouseClick = (event) => {
  ...
  const intersects = raycaster.intersectObjects(
    memoryObjects.filter((obj) => obj.type === "Mesh"),
  );
