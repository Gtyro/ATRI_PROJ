...
class LongTermMemory:
    ...
    def __init__(
        ...
        config: Union[Dict[str, Any], PersonaConfig, None] = None,
    ):
        """初始化长期记忆管理器
        ...
        if isinstance(self.config, PersonaConfig):
            self.decay_rate = self.config.node_decay_rate
        else:
            if "node_decay_rate" not in self.config:
                raise ValueError("node_decay_rate 未配置")
            self.decay_rate = float(self.config["node_decay_rate"])
